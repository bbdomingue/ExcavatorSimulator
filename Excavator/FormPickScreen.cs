...
namespace Excavator
{
    public partial class FormPickScreen : Form
    {
        ...
        private void buttonGo_Click(object sender, EventArgs e)
        {
            ...
            if (r != null)
            {                
                DisplayDevice d = this._Screens[this._ScreensIndex];                
                d.ChangeResolution(r._DisplayResolution);

                const int inc = 1; // Doesnt work at 0 for some unknown reason
                Form f = new Form();
                ...
                f.TopMost = true;
                f.Location = new Point(d.Bounds.Location.X + inc, d.Bounds.Location.Y);
                f.Size = new Size(r._DisplayResolution.Width - inc * 2, r._DisplayResolution.Height);
                f.FormClosed += new FormClosedEventHandler(this.FormClosedMethod);

                f.Show();
                FormBase._FormBase.setFullScreenForm(f);

                this.Close();
            }
        }

        private void FormClosedMethod(object sender, EventArgs e)
        {
            DisplayDevice d = this._Screens[this._ScreensIndex];
            d.RestoreResolution();
        }
