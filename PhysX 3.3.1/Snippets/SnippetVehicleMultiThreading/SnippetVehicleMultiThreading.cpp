...
{
	shdfnd::Sync vehicleRaycastsComplete;
	vehicleRaycastsComplete.reset();
	...
	vehicleRaycastsComplete.wait(shdfnd::Sync::waitForever);
...
{
	shdfnd::Sync vehicleUpdatesComplete;
	vehicleUpdatesComplete.reset();
	...
	//Wait for the signal that the work has been completed.
	vehicleUpdatesComplete.wait(shdfnd::Sync::waitForever);
...
#endif
{
	...
	sprintf(outputString, "Initialising ... \n");
	shdfnd::printString(outputString);
	initPhysics();

	sprintf(outputString, "Simulating ... \n");
	shdfnd::printString(outputString);
